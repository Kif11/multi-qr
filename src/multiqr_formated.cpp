 ...
 public:
  ...
  void save_as_imgs(string output_dir, string image_format = "png") {
    ...
    for (auto qr : m_qrs) {
      ...
      // Add border
      cv::copyMakeBorder(qr_img, qr_img_bordered, m_border_scale,
                         m_border_scale, m_border_scale, m_border_scale,
                         cv::BORDER_CONSTANT, 255);

      // Scale image 5 times
      cv::resize(qr_img_bordered, qr_img_resized, cv::Size(400, 400), 0, 0,
                 cv::INTER_NEAREST);

      cv::imwrite(output_dir + "/" + to_string(index) + "." + image_format,
                  qr_img_resized);
      index++;
    }
  }
